...
- **Error Handling:** Blockchain transactions (e.g., funding an agent node) can fail due to network congestion or insufficient balances. Always implement robust error handling and display clear feedback to the user via toast notifications (`sonner`).
...
- **Admin Access:** The dashboard relies on Freighter Wallet (or similar Stellar wallets) for authentication and transaction signing.
- **API Keys:** Never hardcode sensitive keys or wallet seeds. Use environment variables (`.env`).
- **Supabase:** When interacting with Supabase, ensure RLS (Row Level Security) policies and SSR authentication patterns are respected using `@supabase/ssr`.
